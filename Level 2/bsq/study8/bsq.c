...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/08 12:16:04 by mbani-ya          #+#    #+#             */
/*   Updated: 2025/12/08 14:02:28 by mbani-ya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	element_control(t_map* map, char c1, char c2)
{
	for (int i = 0; i < map->height; i++)
		for (int j = 0; j < map->width; j++)
			if (map->grid[i][j] != c1 && map->grid[i][j] != c2)
				return (-1);
	return (0);
}
...
int	loadMap(FILE* file, t_element* element, t_map* map)
{
	...
	//loop. take line. check \n and remove count. substr. check width
	for (int i = 0; i < map->height; i++)
	{
	...
	if (element_control (map, element->empty, element->obstacle) == -1)
	{
		free(line);
		free_map(map->grid);
		return (-1);
...
//matrix init
//loop check obstacle, check border, use algorithm find min,
//update biggest square
